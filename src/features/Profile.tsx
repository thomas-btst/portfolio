...
function FormationBadge({children}: PropsWithChildren) {
    return <Link to="/career" className="transition hover:scale-105">
...
function Overview({title, children, seeMore}: OverviewProps) {
    return <div className="w-full flex flex-col space-y-4">
        <div className="flex justify-between items-center">
            ...
            <Underline>
                <Link to={seeMore} className="text-purple-800 dark:text-purple-200 text-sm">Voir plus</Link>
            </Underline>
        </div>
        {children}
    </div>
...
function TechnologiesOverview({technologyKeys}: TechnologiesOverviewProps) {
    return <Overview title="Technologies" seeMore="/technologies">
        <div className="flex flex-wrap justify-center gap-4">
            {technologyKeys.map(technoKey => <TechnologyButton key={technoKey} techno={technologies[technoKey]}/>)}
        </div>
    </Overview>
}
...
export function Profile() {
    return <motion.div
        ...
        className="flex flex-col items-center bg-white dark:bg-zinc-700 bg-opacity-75 pt-4 pb-8 px-14 min-w-96 shadow-md space-y-7 rounded-lg text-nowrap"
    >
        ...
        <div className="pt-2">
            <IconButton to="/cv.pdf" target="_blank" icon={faDownload}>Télécharger mon CV</IconButton>
        </div>
    </motion.div>
}