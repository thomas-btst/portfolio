...
export function Bar({ routes }: { routes: { pth: string; name: string }[] }) {
  ...
  return (
    <header className="flex flex-col lg:flex-row space-y-10 lg:space-y-0 relative justify-between items-center py-4 px-4 mb-3 lg:mb-0 select-none">
      {/* Logo & Name */}
      ...
      <div className="items-center gap-x-5 hidden lg:flex">
        <ul className="flex space-x-3">
          {links.map((link, i) => (
            <li
              key={i}
              className="transition-all duration-200 hover:scale-[123%] hover:text-purple-900 dark:hover:text-purple-300"
            >
